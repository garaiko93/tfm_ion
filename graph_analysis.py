...
def filter_graph(study_area_dir, graph_file):
    ...
    #                                            'avg_degree_conectivity', 'avg_edge_density',
    #                                            'avg_shortest_path_duration', 'nodes_betweenness', 'edges_betweenness',
    #                                            'node_load_centrality', 'edge_load_centrality', 'clustering', 'excentrality',
    #                                            'geometry'])
    ...
    for area in study_area_list:
        ...
        if os.path.isfile(str(shp_path) + '/done.txt') == True:
            attr_df = pd.read_csv(str(study_area_dir) + '/' + 'attribute_table.csv', sep=",")
            print('This area was already filtered')
            print('----------------------------------------------------------------------')
            continue
        ...
        f = open(str(shp_path) + '/done.txt', "w+")
        f.write('This area was already filtered')
        f.close()
        print('----------------------------------------------------------------------')
    print('----------------------------------------------------------------------')
...
def topology_attributes(study_area_dir):
    study_area_list = list(os.walk(study_area_dir))[0][1]
    ...
    if os.path.isfile(str(study_area_dir) + '/' + 'attribute_table.csv') == True:
        ...
        print('Attr_df does not exist, df created empty')

    for area in study_area_list:
        if attr_df['area'].str.contains(area).any():
            print(str(area)+' exists in attributes table')
            continue
        ...
        print('Pperiphery nodes: ' + periphery)
        ...
        print('Baryenter nodes: ' + barycenter)
        # ----------------------------------------------------------------------
        # 10.
        ...
        new_row = {'area': area,
                   ...
                   'avg_degree_conectivity': avg_degree_connect_data,
        ...
        attr_df.append(new_row, ignore_index=True)
        print('----------------------------------------------------------------------')
